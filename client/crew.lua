...
AddEventHandler('lsv:playerDisconnected', function(playerName, player)
	local playerIndex = Utils.Index(Player.crewMembers, player)
...
AddEventHandler('lsv:invitedToCrew', function(player)
	if not Utils.IsEmpty(Player.crewMembers) or alreadyInvited then
...
AddEventHandler('lsv:addedCrewMember', function(player, member, members)
	if player ~= GetPlayerServerId(PlayerId()) and GetPlayerServerId(PlayerId()) ~= member and Utils.Index(Player.crewMembers, player) then
