				...
				table.iforeach(_gangData.crews, function(crewData, index)
					if index < 4 then
						...
						Crew.ForEachMember(crewData.id, function(member)
							PlayerData.UpdateCash(member, Settings.gangWars.rewards.top[index].cash, nil, true)
						...
						Crew.ForEachMember(crewData.id, function(member)
							PlayerData.UpdateCash(member, crewData.points * pointCash, nil, true)
							PlayerData.UpdateExperience(member, crewData.points * pointExp, nil, true)
						end)
					end
				end)
			else
