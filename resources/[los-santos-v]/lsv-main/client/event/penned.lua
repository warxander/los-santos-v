...
local function getPlayerPoints()
	local player = table.find_if(_pennedData.players, function(player)
			...
			if Player.IsInFreeroam() then
				...
				for i = barPosition, 1, -1 do
					if _pennedData.players[i] then
						Gui.DrawBar(_playerPositions[i]..GetPlayerName(GetPlayerFromServerId(_pennedData.players[i].id)), _pennedData.players[i].points,
...
AddEventHandler('lsv:finishPennedIn', function(winners)
	...
	local messageText = isPlayerWinner and 'You have won Penned In with a score of '..playerPoints or Gui.GetPlayerName(winners[1], '~p~')..' has won Penned In.'
