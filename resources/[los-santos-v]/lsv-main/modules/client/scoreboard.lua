...
function Scoreboard.DisplayThisFrame()
	...
	for index = 1, math.min(Settings.scoreboardMaxPlayers, Utils.GetTableLength(scoreboard)) do
		...
		-- Draw player name
		local playerColor = Utils.IndexOf(Player.crewMembers, scoreboard[index].id) and Color.GetHudFromBlipColor(Color.BlipLightBlue()) or Color.GetHudFromBlipColor(Color.BlipDarkBlue())
